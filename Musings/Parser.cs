...
namespace TeenyTinyCompiler.Musings
{
    class Parser
    {
        ...
        {
            Print("PROGRAM");
            _emitter.HeaderLine("using System;");
            _emitter.HeaderLine("");
            _emitter.HeaderLine("namespace ProjectNamespace {");
            _emitter.HeaderLine("public class Program {");
            _emitter.HeaderLine("void Main(string[] args){");
        ...
        void Statement()
        {
            if (CheckToken(TokenType.PRINT))
            {
                ...
                {
                    _emitter.EmitLine("Console.WriteLine(\"" + _currentToken.TokenText + "\");");
                    NextToken();
                }
                else
                {
                    _emitter.Emit("Console.WriteLine(string.Format(\"{0}\", ");
                    Expression();
                    _emitter.EmitLine("));");
                }
            }
            ...
            else if (CheckToken(TokenType.INPUT))
            {
                ...
                _emitter.EmitLine("if (!float.TryParse(Console.ReadLine(), out " + _currentToken.TokenText + ")) {");
